(() => {
  ...
  const EMPTY = { students: [], races: [], arrivals: [], tab: 'students', notice: 'Scanner en attente', lastScan: '' };
  const DEMO = [
    ...
    return `<section class="page-grid">
      <div class="card span-2"><div class="card-head"><div><h2>Liste des élèves</h2><p>Importe un fichier Excel, XLSX ou CSV.</p></div><strong>${state.students.length} élèves</strong></div>
        <div class="actions">
          ...
          <button class="button" id="backup" ${state.students.length?'':'disabled'}>Sauvegarde JSON</button>
        </div>
      ...
      <div class="card span-2 table-card"><table><thead><tr><th>Dossard</th><th>Élève</th><th>Classe</th><th>Niveau</th><th>Sexe</th><th>Enseignant</th><th>Course</th></tr></thead><tbody>
        ${state.students.map(s => `<tr><td class="bib">#${s.bib}</td><td>${esc(nameOf(s))}</td><td>${esc(s.className)}</td><td>${esc(s.level)}</td><td>${esc(s.sex)}</td><td>${esc(s.teacher||'—')}</td><td>${esc(s.raceId ? (raceOf(s.raceId)?.name || 'Affectée') : '—')}</td></tr>`).join('')}
      </tbody></table></div>
    </section>`;
  }
  ...
  function racesPage() {
    ...
    return `<section class="page-grid">
      ...
      <div class="card"><h2>Courses</h2><div class="race-list">${state.races.length ? state.races.map(r => {
        ...
        return `<div class="race-row"><div><strong>${esc(r.name)}</strong><span>${runners.length} élèves · ${r.levels.map(esc).join(', ')} · ${r.sexes.join('/')}</span>${r.startedAt ? `<span>Démarrée à ${new Date(r.startedAt).toLocaleTimeString('fr-FR')}</span>` : ''}</div>
          <div class="race-actions">${r.startedAt ? `<span class="started">${done}/${runners.length} arrivés</span>` : `<button class="button danger-ghost" data-delete="${r.id}">Supprimer</button><button class="button start" data-start="${r.id}">DÉPART</button>`}</div></div>`;
  ...
  function timingPage() {
    ...
    const pending = state.students.filter(s => s.raceId && raceOf(s.raceId)?.startedAt && s.elapsedMs == null).length;
    ...
    return `<section class="timing-layout">
      ...
      <div class="card"><h2>Courses actives</h2><div class="race-list">${active.length ? active.map(r => {
        ...
        return `<div class="race-row compact"><div><strong>${esc(r.name)}</strong><span>Départ ${new Date(r.startedAt).toLocaleTimeString('fr-FR')}</span></div><strong>${done}/${runners.length}</strong></div>`;
  ...
  function resultsPage() {
    ...
    return `<section class="page-grid">
      <div class="card span-2"><div class="card-head"><div><h2>Résultats</h2><p>${done} élèves classés. Un élève est présent dès qu’une arrivée est enregistrée.</p></div></div>
      ...
      <div class="card table-card"><h3>Classement des classes</h3><table><thead><tr><th>Niveau</th><th>Rang</th><th>Classe</th><th>Présents</th><th>Moyenne</th></tr></thead><tbody>${g.classAverages.map(r => `<tr><td>${esc(r.level)}</td><td>${r.rank}</td><td>${esc(r.className)}</td><td>${r.count}</td><td>${r.average}</td></tr>`).join('')}</tbody></table></div>
      <div class="card table-card"><h3>Classement des enseignants</h3><table><thead><tr><th>Rang</th><th>Enseignant</th><th>Élèves</th><th>Moyenne</th></tr></thead><tbody>${g.teacherAverages.map(r => `<tr><td>${r.rank}</td><td>${esc(r.teacher)}</td><td>${r.count}</td><td>${r.average}</td></tr>`).join('')}</tbody></table></div>
    </section>`;
  }
  ...
  function loadDemo() {
    ...
    state = { ...EMPTY, students, races:[r6,r5], notice:'Démo chargée : dossards 101 à 112. Lance une course puis scanne.' };
    save(); render();
  }
  ...
  async function importStudents(file) {
    ...
    try {
      ...
      state={...EMPTY,students,notice:`${students.length} élèves importés`}; save(); render();
  ...
  function startRace(id) { const r=raceOf(id); if(!r || r.startedAt) return; if(!confirm(`Lancer maintenant « ${r.name} » ?`)) return; r.startedAt=Date.now(); state.notice=`${r.name} lancée`; state.tab='timing'; save(); render(); }
  ...
  function finish(student, stamp, method) {
    ...
    const race=raceOf(student.raceId); if(!race?.startedAt) return notify(`La course de ${nameOf(student)} n’a pas démarré`);
    const elapsedMs=Math.max(0,stamp-race.startedAt); student.finishedAt=stamp; student.elapsedMs=elapsedMs;
    state.arrivals.push({id:uid(),studentId:student.id,raceId:race.id,stamp,elapsedMs,method}); state.notice=`#${student.bib} ${nameOf(student)} — ${fmt(elapsedMs)}`; save(); render();
  }
  ...
  function rankings() {
    ...
    const levels=new Map(); [...lc.entries()].forEach(([k,a])=>{const[level,className]=k.split('|'); if(!levels.has(level)) levels.set(level,[]); levels.get(level).push({className,count:a.length,averageMs:avg(a.map(s=>s.elapsedMs))});});
    [...levels.entries()].sort().forEach(([level,a])=>{a.sort((x,y)=>x.averageMs-y.averageMs);a.forEach((x,i)=>classAverages.push({level,rank:i+1,className:x.className,count:x.count,average:fmt(x.averageMs)}));});
    const teachers=new Map(); finished.filter(s=>s.teacher).forEach(s=>{if(!teachers.has(s.teacher)) teachers.set(s.teacher,[]); teachers.get(s.teacher).push(s);});
    [...teachers.entries()].map(([teacher,a])=>({teacher,count:a.length,averageMs:avg(a.map(s=>s.elapsedMs))})).sort((a,b)=>a.averageMs-b.averageMs).forEach((x,i)=>teacherAverages.push({rank:i+1,teacher:x.teacher,count:x.count,average:fmt(x.averageMs)}));
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(g.classAverages.map(r=>({Niveau:r.level,Rang:r.rank,Classe:r.className,Présents:r.count,'Temps moyen':r.average}))),'Classement classes');
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(g.teacherAverages.map(r=>({Rang:r.rank,Enseignant:r.teacher,Élèves:r.count,'Temps moyen':r.average}))),'Enseignants');
  ...
  function exportResultsPdf() {
    ...
    pdfTable(doc,'Classement des enseignants',['Rang','Enseignant','Élèves','Temps moyen'],g.teacherAverages.map(r=>[r.rank,r.teacher,r.count,r.average]),false);
