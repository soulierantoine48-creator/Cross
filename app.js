(() => {
  ...
  function resultsPage() {
    ...
    return `<section class="page-grid">
      ...
      <div class="card table-card"><h3>Meilleure classe par niveau</h3><table><thead><tr><th>Niveau</th><th>Rang</th><th>Classe</th><th>Classés</th><th>Moyenne</th></tr></thead><tbody>${g.classAverages.map(r => `<tr><td>${esc(r.level)}</td><td>${r.rank}</td><td>${esc(r.className)}</td><td>${r.count}</td><td>${r.average}</td></tr>`).join('')}</tbody></table></div>
      <div class="card table-card"><h3>Meilleure classe du collège</h3><table><thead><tr><th>Rang</th><th>Classe</th><th>Niveau</th><th>Classés</th><th>Moyenne</th></tr></thead><tbody>${g.classOverall.map(r => `<tr><td>${r.rank}</td><td>${esc(r.className)}</td><td>${esc(r.level)}</td><td>${r.count}</td><td>${r.average}</td></tr>`).join('')}</tbody></table></div>
      <div class="card span-2 table-card"><h3>Classement des professeurs</h3><table><thead><tr><th>Rang</th><th>Professeur</th><th>Classes</th><th>Élèves classés</th><th>Élèves inscrits</th><th>Moyenne</th></tr></thead><tbody>${g.teacherAverages.map(r => `<tr><td>${r.rank}</td><td>${esc(r.teacher)}</td><td>${esc(r.classes.join(', '))}</td><td>${r.count}</td><td>${r.enrolled}</td><td>${r.average}</td></tr>`).join('')}</tbody></table></div>
    </section>`;
  }
  ...
  function rankings() {
    ...
    [...classes.entries()].map(([className,a])=>({className,level:a[0]?.level||'',count:a.length,averageMs:avg(a.map(s=>s.elapsedMs))})).sort((a,b)=>a.averageMs-b.averageMs).forEach((x,i)=>classOverall.push({rank:i+1,className:x.className,level:x.level,count:x.count,average:fmt(x.averageMs)}));
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(g.classAverages.map(r=>({Niveau:r.level,Rang:r.rank,Classe:r.className,Classés:r.count,'Temps moyen':r.average}))),'Classes par niveau');
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(g.classOverall.map(r=>({Rang:r.rank,Classe:r.className,Niveau:r.level,Classés:r.count,'Temps moyen':r.average}))),'Classes collège');
  ...
  function exportResultsPdf() {
    ...
    pdfTable(doc,'Classement des classes par niveau',['Niveau','Rang','Classe','Classés','Temps moyen'],g.classAverages.map(r=>[r.level,r.rank,r.className,r.count,r.average]),false);
    pdfTable(doc,'Classement des classes du collège',['Rang','Classe','Niveau','Classés','Temps moyen'],g.classOverall.map(r=>[r.rank,r.className,r.level,r.count,r.average]),false);
